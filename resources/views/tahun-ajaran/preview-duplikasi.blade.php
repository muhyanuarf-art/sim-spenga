...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    {{-- Guru BK --}}
    <div class="card p-6">
        <p class="font-bold text-slate-800 mb-3">Mapping Guru BK</p>

        <div class="grid sm:grid-cols-2 gap-3 mb-4 text-sm">
            <div class="rounded-lg bg-emerald-50 text-emerald-700 px-3 py-2">
                <span class="font-bold">{{ count($rencana['guru_bk']['disalin']) }}</span> akan disalin
            </div>
            <div class="rounded-lg bg-slate-100 text-slate-500 px-3 py-2">
                <span class="font-bold">{{ count($rencana['guru_bk']['sudah_ada']) }}</span> sudah ada di tujuan (dilewati)
            </div>
        </div>

        @if(count($rencana['guru_bk']['disalin']) > 0)
        <p class="text-xs font-semibold text-slate-500 uppercase mb-1">Akan disalin:</p>
        <ol class="text-sm text-slate-700 list-decimal list-inside space-y-0.5 mb-2 max-h-48 overflow-y-auto">
            @foreach($rencana['guru_bk']['disalin'] as $baris)
                <li>
                    {{ $baris['sumber']->guru->name ?? '-' }} — {{ $baris['kelas_tujuan']->nama_kelas }}
                    @if($baris['kelas_baru']) <span class="text-xs text-amber-600">(kelas baru)</span> @endif
                </li>
            @endforeach
        </ol>
        ...
        @if(count($rencana['guru_bk']['disalin']) === 0 && count($rencana['guru_bk']['sudah_ada']) === 0)
        <p class="text-sm text-slate-400">Tidak ada mapping guru BK di {{ $sumber->nama }} {{ $sumber->semester }}.</p>
        @endif
    </div>
    ...
    @php
        $totalDisalin = count($rencana['kelas']['disalin']) + count($rencana['mengajar']['disalin']) + count($rencana['guru_bk']['disalin']) + count($rencana['jadwal']['disalin']);
    @endphp
