...
<div class="space-y-6">
    ...
    <div class="card p-5 no-print">
        <form method="GET" class="flex flex-wrap items-end gap-3">
            ...
            <button type="button" onclick="cetakBagian('print-pemanggilan')" class="btn-outline">🖨️ Cetak / Export PDF</button>
        </form>
    </div>
    ...
    <div class="card p-5 print-section" id="print-pemanggilan">
        <p class="font-extrabold text-slate-800 text-lg mb-1">Rekap Pemanggilan Orang Tua</p>
            ...
            <table class="table-clean w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-slate-500">{{ \Illuminate\Support\Str::limit($p->alasan, 60) }}</td>
                        <td>{{ $p->ortu_hadir ? 'Hadir' : 'Tidak Hadir' }}</td>
                        <td class="text-slate-500">{{ $p->petugas->name ?? '-' }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="6" class="text-center text-slate-400 py-8">Belum ada pemanggilan orang tua tercatat.</td></tr>
